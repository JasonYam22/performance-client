...
function CaloriePage() {
  ...
  const findNutrientAmount = (nutrientList, nutrientName) => {
    const matchingNutrients = nutrientList.filter((nutrient) => {
      return nutrient.nutrientName === nutrientName;
    });

    if (matchingNutrients.length > 0) {
      return matchingNutrients[0].value;
    } else {
      return 0;
    }
  };

  const loadFoodNutrients = async () => {
    if (!mealName) {
      return;
    }
    ...
    try {
      ...
      setGetFoodNutrients(false);
    } catch (error) {
      console.log(error);
      setErrorMessage("Nutrition lookup failed.");
      setGetFoodNutrients(false);
    }
  };
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-10 lg:px-10">
        ...
        <div className="mb-10 flex items-end justify-between gap-6">
          <div>
            ...
            <p className="mt-3 text-sm text-[#A6ABB2]">{caloriesLeftSum}</p>
          </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
          ...
          <div className="rounded-3xl border-2 border-[#3A3D42] bg-[#1D2025] p-7 md:p-9">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button
                type="button"
                onClick={loadFoodNutrients}
                disabled={getFoodNutrients}
                className="flex items-center gap-2 rounded-full bg-[#FF5A36] px-6 py-2.5 text-sm font-bold text-[#15171B] shadow-[0_0_25px_-6px_#FF5A36] transition-all hover:bg-[#ff7355] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {getFoodNutrients ? "Looking for nutrients" : "Auto-fill nutrition"}
              </button>
          ...
          <div className="rounded-3xl border-2 border-[#3A3D42] bg-[#1D2025] p-7 md:p-9">
            ...
            <div className="mt-6 grid grid-cols-3 gap-2 text-center">
              ...
              <div>
                <p className="text-lg font-bold text-[#FF6384]">{totalFat}g</p>
                <p className="text-[10px] font-bold uppercase tracking-wider text-[#A6ABB2]">
                  Fat
                </p>
              </div>
